    ...
    public class VillaNumberService : BaseService, IVillaNumberService
    {
        ...
        public Task<T> CreateAsync<T>(VillaNumberCreateDTO entity, string token)
        {
            return SendAsync<T>(new APIRequest()
            {
                ...
                Data = entity,
                Url = ApiUrl + $"/api/{SD.CurrentAPIVersion}/VillaNumberAPI",
                Token = token
            });
        }
        ...
        public Task<T> DeleteAsync<T>(int id,string token)
        {
            return SendAsync<T>(new APIRequest()
            {
                ...
                Url = ApiUrl + $"/api/{SD.CurrentAPIVersion}/VillaNumberAPI/" + id,
                Token = token
            });
        }
        ...
        public Task<T> UpdateAsync<T>(VillaNumberUpdateDTO entity, string token)
        {
            return SendAsync<T>(new APIRequest()
            {
                ...
                Data = entity,
                Url = ApiUrl + $"/api/{SD.CurrentAPIVersion}/VillaNumberAPI/" + entity.VillaNo,
                Token = token
            });
        }
    }
